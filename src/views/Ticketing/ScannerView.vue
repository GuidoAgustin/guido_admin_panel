<template>
  <div class="scanner-container bg-dark text-white min-vh-100 d-flex flex-column">
    ...
    <div
      ...
      class="camera-wrapper flex-grow-1 position-relative d-flex flex-column justify-content-center align-items-center p-3"
    >
      <div
        class="qr-box shadow-lg border border-2 border-info rounded overflow-hidden"
        style="width: 100%; max-width: 400px; aspect-ratio: 1/1"
      >
        <qrcode-stream @detect="onDetect" @error="onError" :paused="cargando">
          <div
            v-if="cargando"
            class="loading-overlay d-flex justify-content-center align-items-center w-100 h-100 bg-dark bg-opacity-75"
          >
            <div
      ...
      <div class="manual-input mt-auto w-100" style="max-width: 400px">
        ...
        <div class="input-group input-group-lg">
          <input
            ...
            type="text"
            class="form-control form-control-lg bg-white text-dark fw-bold text-center border-2 border-info shadow-sm"
            placeholder="Ej: CA19BC80"
    ...
    <div
      v-else
      class="result-screen flex-grow-1 d-flex flex-column justify-content-center align-items-center p-4 text-center"
      :class="colorFondo"
    >
      ...
      <div
        v-if="tipoEntrada"
        class="ticket-type bg-black bg-opacity-25 px-4 py-2 rounded-pill fs-4 fw-bold mb-5 border border-light"
      >
        Tipo: {{ tipoEntrada }}
      </div>

      <button
        class="btn btn-light btn-lg w-100 fw-bold shadow-lg mt-auto mb-4"
        style="max-width: 350px; padding: 20px"
        @click="reiniciarEscaner"
      >
