...
export default {
  ...
  methods: {
    ...mapActions(['createEvento', 'createTipo', 'fetchTickets']),
    ...
    async submit() {
      try {
        ...
        // Crear evento y obtener respuesta completa
        const response = await this.createEvento(payload)

        // Extraer ID de evento
        const evento_id = response.data?.data?.evento_id || response.data?.evento_id
        if (!evento_id) throw new Error('No se recibió el ID del evento')

        // Crear tipos asociados
        ...
        console.error('Error en submit de ModalForm:', err)
        this.$toast.error(err.message || 'Error al crear evento o tipos')
      }
    }
  }
}
